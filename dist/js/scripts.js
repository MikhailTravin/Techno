...
const headerTop = document.querySelector('.top-header__right');
if (iconMenu) {
...
function adaptPortfolioForMobile() {
    const portfolioSlider = document.querySelector('.block-portfolio__slider');
    if (!portfolioSlider) return;

    const isMobile = window.matchMedia('(max-width: 992px)').matches;
    const slidesWrappers = portfolioSlider.querySelectorAll('.block-portfolio__slides');

    if (isMobile && slidesWrappers.length > 0) {
        slidesWrappers.forEach(wrapper => {
            const innerSlides = wrapper.querySelectorAll('.block-portfolio__slide');
            innerSlides.forEach(slide => {
                slide.classList.add('swiper-slide');
                wrapper.parentNode.insertBefore(slide, wrapper.nextSibling);
            });
            wrapper.remove();
        });
    }
...
if (document.querySelector('.block-equipment__slider')) {
    const swiperEquipment = new Swiper('.block-equipment__slider', {
        observer: true,
        observeParents: true,
        slidesPerView: 'auto',
        spaceBetween: 20,
